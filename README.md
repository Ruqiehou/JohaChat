...
#    client = MessageClient(ws_url="ws://localhost:3002", access_token="")
...
WebSocket 连接参数直接在 `main.py` 的 MessageClient 构造函数中设置：
...
```
run.py                     ← 统一启动入口：启动 MessageClient，注册群消息事件
  │
  └─ joha/                 ← 核心包
       ├─ adapter/         ← NapCat 适配层
...
```
群消息 → NapCatQQ
         → WebSocket → MessageClient (NapCatClient)
